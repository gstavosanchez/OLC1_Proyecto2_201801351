...
export class AnalizadorLexico {
    ...
    private isLetter(cararter: string): boolean {
        ...
        return (expresion.test(cararter.toLowerCase()));
    ...
    private getSizeCommentUn(index: number): number {
        let longitud: number = 0;
        for (index; index < this.entrada.length; index++) {
            const c: string = this.entrada.charAt(index);
            if (c == `\n`) {
    ...
    private getSizeCommentMult(index: number): number {
        let longitud: number = 0;
        for (index; index < this.entrada.length; index++) {
            const c: string = this.entrada.charAt(index);
            if (c == `\n`) {
            ...
            if (c == "*" && this.entrada.charAt(index + 1) == "/") {
                break;
            }
            longitud++;
        }
    ...
    private q0(): void {
        ...
        while (index < this.entrada.length) {
            ...
            if (caracterActual == "{") {
                ...
                this.setTokenList(TypeToken.DIFERENTE, "!=");
                index++;
            }else if(caracterActual == "!"){
                this.setTokenList(TypeToken.NOT,caracterActual)
            } else if (caracterActual == "-") {
                this.setTokenList(TypeToken.MENOR, caracterActual);
            }else if(caracterActual == "."){
                this.setTokenList(TypeToken.PUNTO, caracterActual);
            } else if (caracterActual == "<" && this.entrada.charAt(index + 1) == "=") {
                ...
                this.setTokenList(TypeToken.POR,caracterActual);
            } else if (caracterActual == ",") {
                ...
                const sizeLex: number = this.getSizeCommentUn(index);
                this.q5(index, index + sizeLex);
                index += sizeLex;
                continue
            } else if (caracterActual == "/" && this.entrada.charAt(index + 1) == "*") {
                const sizeLex:number = this.getSizeCommentMult(index);
                this.q5(index,index + sizeLex+1);
                index += sizeLex +1;
                continue
            }else if(caracterActual == "/"){
                this.setTokenList(TypeToken.DIVISION, caracterActual);
    ...
    private q5(actual: number, fin: number) {
        const lexema: string = this.entrada.substr(actual, fin);
        this.column += (lexema.length);
    }
